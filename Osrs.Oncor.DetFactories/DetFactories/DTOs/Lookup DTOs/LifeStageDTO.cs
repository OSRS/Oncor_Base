using System.Collections.Generic;

namespace Osrs.Oncor.DetFactories.DTOs.Lookup_DTOs
{
    public class LifeStageDTO : BaseDTO, IKeyed
    {
        public string Key { get; set; }
        public string Name { get; set; }
        //public string Description { get; set; }
        //public string InternalId { get; set; }

        public LifeStageDTO()
        {
        }

        public LifeStageDTO(Dictionary<string, string> values)
        {
            Schema schema = Schema;
            schema.ValidationIssues = ValidationIssues;
            Key = (string)schema.Parse(values, "Key");
            Name = (string)schema.Parse(values, "Name");
            //Description = (string)schema.Parse(values, "Description");
            //InternalId = (string)schema.Parse(values, "InternalId");
        }

        public override Schema Schema => GetSchema();

        public static Schema GetSchema()
        {
            Schema schema = new Schema(SchemaType.LookupSchema, "life stage");
            schema.Add("Key", typeof(string), SchemaEntryType.LocalLookupKey, 1000);
            schema.Add("Name", typeof(string), SchemaEntryType.Normal);
            //schema.Add("Description", typeof(string), SchemaEntryType.Normal);
            //schema.Add("InternalId", typeof(string), SchemaEntryType.Normal);
            return schema;
        }

        public override Dictionary<string, string> Values()
        {
            Dictionary<string, string> result = new Dictionary<string, string>();
            result.Add("Key", Key);
            result.Add("Name", Name);
            //result.Add("Description", Description);
            //result.Add("InternalId", InternalId);
            return result;
        }
        public override void Validate()
        {
            Schema schema = Schema;
            schema.ValidationIssues = ValidationIssues;
            schema.ValidateField(Key, "Key");
            schema.ValidateField(Name, "Name");
            //schema.ValidateField(Description, "Description");
            //schema.ValidateField(InternalId, "InternalId");
        }

        public string LookupKey => Key;
    }
}